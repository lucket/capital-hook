...
- **Payload Setup**: Define custom JSON payloads for incoming TradingView webhooks to perfectly match your strategy's needs.
    ...
    - **Market Closed Handling (`MKT_CLOSED`)**: Automatically manages trades when the market is about to close. If enabled, the hook will close any open position for the instrument 2 minutes before the market closes, ensuring positions are not left open during market downtime.
